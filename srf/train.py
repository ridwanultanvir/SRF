...
from srf.config import BaseOptions
from srf.start_end_dataset import \
    StartEndDataset, start_end_collate, prepare_batch_inputs
from srf.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from srf.inference import eval_epoch, start_inference, setup_model
from utils.basic_utils import AverageMeter, dict_to_markdown
from utils.model_utils import count_parameters

from srf.loss_fun.VTCLoss import VTCLoss
from srf.loss_fun.CTCLoss import CTC_Loss
from srf.loss_fun.VTCLoss_caption import VTCLoss_caption
import logging
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        src_txt_mask, src_vid_mask, src_cap_mask = model_inputs['src_txt_mask'], model_inputs['src_vid_mask'], model_inputs['src_cap_mask']
        src_cap_mask = (src_cap_mask.sum(dim=2) > 0).float()
        pos_mask =  targets['src_pos_mask'] 

        src_txt_ed, src_vid_ed, src_cap_ed =  outputs['src_txt_ed'], outputs['src_vid_ed'], outputs['src_cap_ed']
        src_vid_cls_ed, src_txt_cls_ed = outputs['src_vid_cls_ed'], outputs['src_txt_cls_ed']
        ...
        loss_align_VTCcaption = VTCLoss_caption()
        loss_vid_cap_align_VTC = loss_align_VTCcaption(src_cap_ed, src_vid_ed)
        
        losses = losses + opt.VTC_loss_coef*(loss_vid_txt_align_VTC) + opt.caption_loss_coef*(loss_vid_cap_align_VTC) + opt.CTC_loss_coef*(loss_vid_txt_align)
