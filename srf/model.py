...
from srf.span_utils import generalized_temporal_iou, span_cxw_to_xx

from srf.matcher import build_matcher
from srf.transformer import build_transformer
from srf.position_encoding import build_position_encoding
from srf.misc import accuracy
from srf.interaction.test_CQA import VSLFuser
import torchvision
...
class CrossAttention(nn.Module):
    ...
    def forward(self, src1, src2, mask=None):
        """
        src1: Tensor of shape (batch_size, seq_len1, d_model)
        src2: Tensor of shape (batch_size, seq_len2, d_model)
        mask1: Attention mask for src1 (optional)
        mask2: Attention mask for src2 (optional)
        """
        # Cross-attention: src1 queries, src2 keys and values
        src1 = src1.permute(1, 0, 2) # (seq_len1, batch_size, d_model)
        src2 = src2.permute(1, 0, 2) # (seq_len2, batch_size, d_model)
        attn_output, _ = self.cross_attn(query=src1, key=src2, value=src2, key_padding_mask=mask)
        src1 = src1 + self.dropout(attn_output)
        src1 = self.norm(src1)

        # Optionally add a feedforward network (FFN) layer
        src1_ffn = self.ffn(src1)
        src1 = src1 + self.dropout(src1_ffn)
        src1 = self.norm(src1)

        src1 = src1.permute(1, 0, 2) # (batch_size, seq_len1, d_model)
        return src1
...
class SelfAttention(nn.Module):
    ...
    def forward(self, src, mask=None):
        ...
        mask: Attention mask for src (optional)
        """
        # Self-attention: src is used as query, key, and value
        src = src.permute(1, 0, 2)  # (seq_len, batch_size, d_model)
        attn_output, _ = self.self_attn(query=src, key=src, value=src, key_padding_mask=mask)

        src = src + self.dropout(attn_output)  # Add & Norm
        src = self.norm(src)

        # Feedforward network (FFN) layer
        src_ffn = self.ffn(src)
        src = src + self.dropout(src_ffn)  # Add & Norm
        src = self.norm(src)
...
class SRF(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim, cap_dim,
                 num_queries, input_dropout, aux_loss=False,
                 contrastive_align_loss=False, contrastive_hdim=64,
                 max_v_l=75, span_loss_type="l1", use_txt_pos=False, n_input_proj=2, aud_dim=0, clip_len = 2, iter_num = 5):
        ...
        Parameters:
            ...
            cap_dim: int, caption feature input dimension
        ...
        self.input_cap_proj = nn.Sequential(*[
            LinearLayer_caption(cap_dim, hidden_dim, layer_norm=True, dropout=input_dropout, relu=relu_args[0]),
            LinearLayer_caption(hidden_dim, hidden_dim, layer_norm=True, dropout=input_dropout, relu=relu_args[1]),
            LinearLayer_caption(hidden_dim, hidden_dim, layer_norm=True, dropout=input_dropout, relu=relu_args[2])
        ][:n_input_proj])
        self.weighted_vid_cap_proj = LinearLayer(hidden_dim * 2, hidden_dim, layer_norm=True, dropout=input_dropout, relu=True)
        ...
        self.fuser1 = VSLFuser(transformer.d_model)
        self.fuser2 = VSLFuser(transformer.d_model)
  
        self.gru_extractor = GRUFeatureExtractor(hidden_dim, hidden_dim, num_layers=1, bidirectional=False)
        self.cross_attention = CrossAttention(256, 8, dropout=0.1)
        self.self_attention = SelfAttention(256, 8, dropout=0.1)
        self.iter_num = iter_num
        self.weight_number = nn.Parameter(torch.rand(1, 1, 75))



    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_cap, src_cap_mask, src_aud=None, src_aud_mask=None):
        """The forward expects two tensors:
               ...
               - src_vid_mask: [batch_size, L_vid], containing 0 on padded pixels,
                    will convert to 1 as padding later for transformer
               - src_cap: [batch_size, L_vid, L_cap, D_cap]
               - src_cap_mask: [batch_size, L_vid, L_cap], containing 0 on padded pixels,
                    will convert to 1 as padding later for transformer
        ...
        src_cap = self.input_cap_proj(src_cap)
        refined_src_cap = self.attention_weighted_sum(src_cap, src_txt)
        refined_src_cap_mask = (src_cap_mask.sum(dim=2) > 0).float()
        
        # CTC VTC
        src_txt_ed = src_txt
        src_vid_ed = src_vid
        src_cap_ed = src_cap.mean(2)
        src_vid_cls_ed = src_vid.mean(1)
        src_txt_cls_ed = src_txt.mean(1)

        src_txt = self.self_attention(src_txt, src_txt_mask)
        
        src_vid2 = self.fuser1(src_vid, src_txt, src_vid_mask, src_txt_mask)
        src_cap2 = self.fuser2(refined_src_cap, src_txt, refined_src_cap_mask, src_txt_mask)

        x_vid_cap = self.cross_attention(src_vid2, src_cap2, refined_src_cap_mask)
        x_cap_vid = self.cross_attention(src_cap2, src_vid2, src_vid_mask)

        x_vid_cap = self.filtering(src_txt, x_vid_cap, x_cap_vid)

        # concat vid and caption
        src_vid = torch.cat([x_vid_cap, x_cap_vid], dim=2)
        src_vid = self.weighted_vid_cap_proj(src_vid)
    ...
    def filtering(self, query, video, caption):
        # query: (bsz, Lt, d)
        # video: (bsz, Lv, d)
        # caption: (bsz, Lv, d)
        b, lv, d = caption.size()
        ...
        src_txt_g = query.mean(1)
        txt_g_norm = src_txt_g / src_txt_g.norm(dim=-1, keepdim=True)
        txt_norm = query / query.norm(dim=-1, keepdim=True)

        q_mean = torch.bmm(txt_norm, txt_g_norm.unsqueeze(-1)).squeeze(-1)

        qv_similarity = torch.matmul(query, video.transpose(1, 2))
        qc_similarity = torch.matmul(query, caption.transpose(1, 2))
        qvc_similarity = self.weight_number * qv_similarity + (1 - self.weight_number) * qc_similarity 

        _, top_indices = torch.topk(q_mean, lt, dim=-1)

        for i in range(self.iter_num):
            if i>=lt:
                break
            top_idx = top_indices[:, i]
            qvc_weights = torch.gather(qvc_similarity, 1, top_idx.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, lv))
            qvc_weights = qvc_weights.squeeze(1)
            qvc_weights = F.softmax(qvc_weights, dim=-1) 

            video = video + video * qvc_weights.unsqueeze(-1) # [batch_size, Lv, d]
        
        return video
        
    
    def attention_weighted_sum(self, caption_feature, text_query_feature):
        # caption_feature: [batch_size, L_vid, L_cap, d]
        # text_query_feature: [batch_size, L_txt, d]

        text_query_feature = text_query_feature.unsqueeze(1)
       
        similarity = torch.matmul(caption_feature, text_query_feature.transpose(-1, -2))
        attention_weights = F.softmax(similarity.mean(dim=3), dim=2)
        attention_weights = attention_weights.unsqueeze(3)
        weighted_caption = caption_feature * attention_weights  
        caption_feature_pooled = weighted_caption.sum(dim=2)  # [batch_size, L_vid, d]
        
        return caption_feature_pooled
    ...
    def MR2HD(self, out, memory, video_length, src_vid_ed):
        ...
        sliced_features_global_expanded = self.gru_extractor(sliced_features).unsqueeze(1) # torch.Size([32, 256])
...
class SetCriterion(nn.Module):
    ...
    # HD loss(margins(saliency) loss + ranking aware contrastive loss)
...
class LinearLayer_caption(nn.Module):
    """Linear layer configurable with layer normalization, dropout, ReLU for 4D input tensors."""

    def __init__(self, in_hsz, out_hsz, layer_norm=True, dropout=0.1, relu=True):
        super(LinearLayer_caption, self).__init__()
        self.relu = relu
        self.layer_norm = layer_norm
        if layer_norm:
            self.LayerNorm = nn.LayerNorm(in_hsz)
    ...
    def forward(self, x):
        """x: (batch, L_vid, L_caption, D_in)"""
        orig_shape = x.shape  # (batch, L_vid, L_caption, D_in)
        x = x.view(-1, orig_shape[-1])  # (batch * L_vid * L_caption, D_in)
        
        if self.layer_norm:
            x = self.LayerNorm(x)
        x = self.net(x)  # (batch * L_vid * L_caption, D_out)
        
        if self.relu:
            x = F.relu(x, inplace=True)
        
        # 원래 차원으로 복원 (batch, L_vid, L_caption, D_out)
        x = x.view(orig_shape[0], orig_shape[1], orig_shape[2], -1)
        return x
