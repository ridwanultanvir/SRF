...
from srf.span_utils import span_xx_to_cxw
...
class StartEndDataset_audio(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir, c_feat_dir, a_feat_dir=None,
    ...
    def __getitem__(self, index):
        ...
        model_inputs["video_caption_feat"] = self._get_caption_feat_by_qid(meta["qid"])  # (Dq, ) or (Lq, Dq)
        if self.use_video:
    ...
    def _get_caption_feat_by_qid(self, qid):
        ...
        # with np.load(c_feat_path) as data:
        #     print("Available keys:", list(data.keys()))
        #print(c_feat.shape)

        # padding addition: if n > 75, (n, 32, 512) → (75, 32, 512)
        if c_feat.shape[0] < 75:
            pad_width = ((0, 75 - c_feat.shape[0]), (0, 0), (0, 0))
        ...
        return torch.from_numpy(c_feat)  # return (Lv, Lc, D)
...
def create_caption_mask(src_caption_tensor, max_frames=75):
    # src_caption_tensor: [actual_frames, Lc, D]
